...
export const ExchangeHistory = () => {
  ...
  const supportedCoinOptions = [
    ...SUPPORTED_COINS.filter((item) => item.id !== 'vndt').map(
  ...
  const filterHistory = () => {
    ...
    const result = searchData(exchangeHistory, keywords);

    if (!isEmpty(searchCoin)) {
      filters[`currencyCode`] = (currencyCode) => {
        if (searchCoin.id === 'all') {
          return currencyCode === 1 || 3 || 17 || 6 || 8 || 10 || 11;
        } else {
  ...
  const renderHistoryItem = ({ item, index }) => {
    ...
    const statusFilter = HISTORY_EXCHANGE_STATUS.find((item) => item.value === status);

    const onCheck = () => {
      Linking.openURL(CHECK_EXPORT + `${hash}`)
    }

    const getFeeCoin = ({ type }) => {
      const findFee = TRANSACTION_TYPE.find((item) => item.value === type);
      if (findFee.value === 1) {
        return  `${formatCommaNumber(fee)} ${selectedCoin?.id.toUpperCase()}`;
      } else {
        return '0 VNDT';
      }
    }
          ...
          <View style={{ flexDirection: 'row', marginTop: 5 }}>
            ...
            <View style={{ justifyContent: 'flex-end' }}>
              <Text
                ...
                {getFeeCoin({ type: transactionType })}</Text>
            </View>
          </View>
