...
import { showLoading, CREDIT_TYPE, ICO_SEARCH, STATUS_CREDIT, ICO_LIST_GIFCODE, searchKeysCredit, Routes } from '@/common';
import dayjs from 'dayjs';
import FontAwesome5 from 'react-native-vector-icons/FontAwesome5';
import ActionSheet from 'react-native-actionsheet';
import { isEmpty } from 'lodash'
import { NavigationService } from '@/services';
import { formatNumberFee, multiFilter } from '@/utils';

export const CreditManagement = () => {
  const { t } = useTranslation();
  const { listCreditManagement, getListcreditManagement } = useAppContext();
  const [listcredit, setListCredit] = useState(null);
  ...
  useEffect(() => {
    filterCredit();
  }, [listCreditManagement, keywords, searchType]);
  ...
  const fetchData = async ({ isShowLoading }) => {
    ...
    const params = { type: 'lender' }
    await getListcreditManagement(params);
  ...
  const filterCredit = () => {
    ...
    const result = searchData(listCreditManagement, keywords);
    ...
    const filtered = multiFilter({ arr: listCreditManagement, filters: filters });
  ...
  const renderCreditItem = ({ item, index }) => {
    ...
    const onListCreditDetail = () => {
      NavigationService.navigate(Routes.CREDITDETAIL, { id: id, typed: CREDIT_TYPE.MANAGEMANT });
    }
        ...
        <TouchableWithoutFeedback onPress={onshowCredit}>
          <View style={styles.itemLeftContainer}>
            ...
            <View style={{ flexDirection: 'row', marginTop: 5 }}>
              <View style={{ flex: 1, justifyContent: 'flex-start' }}>
                <View style={styles.row}>
                  <Text style={[styles.title, { fontWeight: 'bold' }]}>{`${t('brrower')}:`}</Text>
                </View>
              </View>
              <View style={{ justifyContent: 'flex-end' }}>
                <Text style={[styles.title, { marginLeft: 10 }]} numberOfLines={1}>{borrowerEmail}</Text>
              </View>
            </View>
